...
def _unroll_rnn(x, x_name, T, rnn_graph, hidden_size, average_on_relu, reuse):
  """
  ...
  for t in range(T):
    ...
    for s_name in all_tensors[t].keys():
      # Finalize state, apply relu and scale.
      in_tensor = all_tensors[t][s_name]  # 这个s_name不是个字典吗？？？？？？
      ...
      for edge in edges:
        ...
        if s_name == e_name1:
          if e_delay + t < T:
            try:
              out_tensor = e_fun(in_tensor, name=e_name1, hidden_size=hidden_size, reuse=reuse)
              # out_tensor = e_fun(in_tensor, reuse=reuse)
            except ValueError:
              # Sharing if exists.
              out_tensor = e_fun(in_tensor, name=e_name1, hidden_size=hidden_size, reuse=True)
...
def _conv_upsample(_input, name, hidden_size, reuse):
...
def _downsample(_input, name, hidden_size, reuse):
...
def _conv_link(_input, name, hidden_size, reuse):
  with tf.variable_scope(name, auxiliary_name_scope=False):  # 一定要加后面那个参数，不然会出现多次定义的错误，我都不知道这作者怎么特么跑的。
...
def _skip_link(_input, name, hidden_size, reuse):
  return _input
...
def build_model(x, scale, training, reuse):
  ...
  with tf.variable_scope('InputNet'):
    s1_0 = tf.layers.conv2d(
        x,
        hidden_size / 2,  # 生成特征的卷积是64个，后面的都是128，论文在实验部分提到了
        3,
        trainable=training,
        activation=tf.nn.relu,
        name='in_0',
  ...
  with tf.variable_scope('RecurrentNet'):
    full_net_states = _unroll_rnn(
        s1_0, 's1', T, (node_names, edges), hidden_size, average_on_relu=False, reuse=reuse)
  ...
  out = tf.layers.conv2d(
      pre_out, 3, 1, activation=None, name='out', reuse=reuse)  # 不用进行0填充吗？？？
  return out
